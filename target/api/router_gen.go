...
type FuncInterface interface {
	...
	//
	// Lightweight cross-key snapshot containing each key, latest version, operational status, source classification, and source availability. Release notes and artifact lists are omitted. The body is served from the metadata cache and does not perform a live storage scan. Query parameters are rejected.
	...
	//
	// Returns the Go proxy `@latest` JSON document for the newest active canonical version in the requested major line. Major-version paths such as `/{key}/v2/@latest` are accepted and mapped to the same operation. The route is available only when content detection has published a Go overlay for the key and major path.
	...
	//
	// Returns the Go proxy `@v/list` body: one canonical version per line, newest first. Major-version paths such as `/{key}/v2/@v/list` are accepted and mapped to the same operation. The route is available only for keys and major lines where content detection has published a Go overlay. Query parameters are rejected.
	...
	//
	// Lightweight release snapshot for one key. Each item contains version, time, size, and a link to the detail document; release notes and artifact lists are intentionally omitted. Pagination uses keyset cursors: `?after=<next-from-previous-response>`. The `next` field is omitted on the last page.
	...
	// GetMetricsYgg handles GET /metrics/ygg.
	//
	// Yggdrasil mesh peer metrics
	//
...
func (obj *HandlerObj) GetMetricsYgg(ctx context.Context) (GetMetricsYggRes, error) {
	return obj.FuncObj.GetMetricsYgg(ctx)
}
